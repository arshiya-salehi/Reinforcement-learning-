...
class ScalingEnv(gym.Env):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Instance limits and capacity are needed to build the observation space
        ...
        low = numpy.zeros(self.observation_size, dtype=numpy.float32)
        high = numpy.array(
            [
                1.0,
                1.0,
                self.max_instances * self.capacity_per_instance,
                numpy.inf,
                numpy.inf,
            ],
            dtype=numpy.float32,
    ...
    def step(self, action):
        ...
        terminated = bool(self.queue_size > self.max_influx * 10)  # Ensure plain Python bool
        truncated = False  # New gym API requires both terminated and truncated
    ...
    def reset(self, seed=None, options=None):
        ...
        observation = self.__get_observation()
        # Return tuple (obs, info) for newer gym versions, or just obs for older versions
        info = {}
    ...
    def __get_observation(self):
        observation = numpy.zeros(self.observation_size, dtype=numpy.float32)
