...
from src.config import DQN_TOTAL_TIMESTEPS, DQN_CONFIG, MODELS_DIR, LOGS_DIR, RANDOM_SEED
...
def create_env_with_load_pattern(load_pattern='RANDOM'):
    ...
    def _init():
        ...
        # Determine the underlying (unwrapped) environment so we can set options
        inner = env
        # Unwrap any nested wrappers to reach the original env
        ...
        if load_pattern in LOAD_PATTERNS:
            ...
            inner.scaling_env_options['input'] = {
                'function': load_func,
                'options': {}
            }
        elif load_pattern == 'SINE_CURVE':
            inner.scaling_env_options['input'] = INPUTS['SINE_CURVE']
        else:
            inner.scaling_env_options['input'] = INPUTS['RANDOM']
        return env
    return _init


def train_dqn(load_pattern='RANDOM', model_name=None, total_timesteps=None):
    """
    Train DQN agent
    ...
    print(f"Training DQN agent with load pattern: {load_pattern}")
